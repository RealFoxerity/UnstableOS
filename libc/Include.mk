...
LIBC_LIB := $(LIBC_ROOT)/build/libc.a
...
LIBC_CRT0 := $(LIBC_ROOT)/build/src/crt0.s.o
LIBC_CRTI := $(LIBC_ROOT)/build/src/crti.s.o
LIBC_CRTN := $(LIBC_ROOT)/build/src/crtn.s.o
...
LIBC_OBJS := $(patsubst $(LIBC_ROOT)/%, $(LIBC_ROOT)/build/%.o, $(LIBC_SRCS))
LIBC_OBJS_CRT := $(patsubst $(LIBC_ROOT)/%, $(LIBC_ROOT)/build/%.o, $(LIBC_SRCS_CRT))
...
$(LIBC_ROOT)/build/%.c.o: $(LIBC_ROOT)/%.c
	@$(PROGRESS_LABEL) Compiling $(patsubst $(MAKE_ROOT)/%,%,$(abspath $@))
	@mkdir -p $(dir $@)
	@$(CC) $(LIBC_CFLAGS) -c $< -o $@

$(LIBC_ROOT)/build/%.s.o: $(LIBC_ROOT)/%.s
