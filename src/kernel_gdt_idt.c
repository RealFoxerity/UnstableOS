...
void construct_descriptor_tables() {
    ...
    *(struct dt_descriptor*)gdt_descriptor_entries = (struct dt_descriptor) { // using the NULL descriptor for GDTR, not really needed but meh
    ...
    for (int i = 0; i < RES_INTERR_EXCEPTION_COUNT; i++) {
        ...
        // most of our ISRs are short and expect disabled interruts -> interrupt gate
    }
    ...
    //paging_change_flags(idt_descriptor_entries, sizeof(struct idt_gate) * IDT_INTERR_VECTOR_COUNT, 0); // can't protect the idt if it's dynamically allocated because it will never be on a page boundary, maybe if I allocated around 8k and centered it onto a page boundary?
}